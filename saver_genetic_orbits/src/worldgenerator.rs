...
impl Plugin for WorldGeneratorPlugin {
    fn build(&self, app: &mut AppBuilder) {
        app.add_system_set(
            SystemSet::on_enter(GeneratorState::Generate).with_system(generate_world.system()),
        );
    }
}

fn generate_world(mut state: ResMut<State<GeneratorState>>, config: Res<GeneratorConfig>) {
    info!("Generating world");
    if let Err(err) = state.set(GeneratorState::Run) {
