    ...
    fn to_sql(&self) -> Result<ToSqlOutput, SqlError> {
        if self.0 <= i64::max_value() as u64 {
            ...
            Err(SqlError::ToSqlConversionFailure(
                format!(
                    "Value {} is too large for SQLite, max is {}",
                    self.0,
                    i64::max_value(),
                )
                .into(),
...
mod tests {
    ...
    fn test_creates_index() {
        ...
        let (idxname, unique): (String, bool) = storage
            .conn
            .query_row("PRAGMA INDEX_LIST('scenario')", NO_PARAMS, |row| {
                (row.get(1), row.get(2))
            })
            .unwrap();
    ...
    fn test_add_child() {
        ...
        assert_eq!(
            ...
            (
                scenario.id as i64,
                parent.family as i64,
                Some(parent.id as i64),
                (parent.generation + 1) as i64,
                world,
                987.
            ),
        );
